...
|   CreateList (j,new) = CreateList(j-1,new@[("-",10000000)]);
...
fun moredeli (infile)=
	let
		...
		val Prevlist = CreateArray(N,M,nil)		(*Δημιουργω τις Prevlist που είναι λίστα απο λιστες με τουπλες (m,cost)*)
		...
		val Q = [(starti,startj,0)]			(*Δημιουργω την Qlist με αρχικό κόμβο (i=starti,j=startj,cost=0)*)
		val newPrevlist = WriteToPrevious(Prevlist,starti,startj,"n",0,nil,false)	(*Γραφω στην Prevlist στο (i=starti,j=startj) την τιμή ("n",0)*)
